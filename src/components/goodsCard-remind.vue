...
export default {
  ...
  methods: {
    to_detail(id) {
      if(!this.card_data.is_disable){
        this.card_data.is_disable = true;
      }else {
        this.card_data.is_disable = false;
      }
      // console.log(id);
      //  this.$router.push({name:'detail',query:{id:this.card_data.goods_id}})
    }
  }
};
</script>
